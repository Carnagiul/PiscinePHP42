...
function moyenne($csv)
{
	...
	foreach ($csv as $user)
	{
		if ($user["Note"] != "" && $user["Note"] != "Note" && $user["Noteur"] != "moulinette")
		{
			$c += intval($user["Note"]);
			$i++;
		}
...
function moyenne_user($csv)
{
	...
	foreach ($csv as $user)
	{
		if ($user["User"] != "User" && $user["Note"] != "" && $user["Noteur"] != "moulinette")
		{
			$data[$user["User"]]["Name"] = $user["User"];
			$data[$user["User"]]["Note"] += $user["Note"];
			$data[$user["User"]]["NoteI"][$data[$user["User"]]["Count"]] = $user["Note"];
...
function ecart_moulinette($csv)
{
	...
	foreach ($csv as $user)
	{
		if ($user["User"] != "User" && $user["Note"] != "")
		{
			$data[$user["User"]]["Name"] = $user["User"];
			if ($user["Noteur"] == "moulinette")
			{
				$data[$user["User"]]["NoteM"] += $user["Note"];
			}
			else
			{
				$data[$user["User"]]["Count"] += 1;
				$data[$user["User"]]["NoteI"][] = $user["Note"];
				$data[$user["User"]]["Note"] += $user["Note"];
			}
		}
	}
	if (isset($data) && $data != NULL)
	{
		sort($data);
		foreach ($data as $user)
			echo $user["Name"] . ":" . (- ($user["NoteM"] / 1) + ($user["Note"] / $user["Count"])) . "\n";
	}
}
