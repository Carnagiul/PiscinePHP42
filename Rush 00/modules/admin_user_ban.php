...
if (isset($_SESSION['user']) && $_SESSION['user']['admin'] == 1)
{
    if (isset($_GET['user_id']) && $_GET['user_id'] != '' && $_GET['user_id'] != $_SESSION['user']['id'])
    {
        ...
        {
            sql_update("UPDATE `" . TABLE_USER . "` SET `banni` = '1' WHERE `users`.`id` = '" . intval($_GET['user_id']) . "'; ");
            $page = "Cette utilisateur a ete banni";
        }
        else
            $page = "Cette utilisateur ne peut etre banni";
    }
    else
    {
